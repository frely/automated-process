...
func writeSql(data []byte) {
	...
	for _, i := range p.Response.SummaryDetail {
		sqlData := fmt.Sprintf(`INSERT INTO "%s"("Month", "Business", "CashPayAmount", "GroupKey", "GroupValue", "IncentivePayAmount", "RealTotalCost", "TotalCost", "TransferPayAmount", "VoucherPayAmount") VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')`,
			i.GroupKey,
			billMonth,
			i.Business,
