...
func CheckSqlTable() {
	...
	rows, err := db.Query(`select count(*) from pg_class where relname = 'aliyunDescribeInstances';`)
	defer rows.Close()
	if err != nil {
	...
	for rows.Next() {
		...
		if count == "1" {
			log.Println("清空表")
			sqlData := `TRUNCATE TABLE "aliyunDescribeInstances"`
			...
			log.Println("创建表：aliyunDescribeInstances")
			sqlData := `
				CREATE TABLE "public"."aliyunDescribeInstances" (
			...
			for _, v := range commitList {
				sqlData := fmt.Sprintf(`COMMENT ON COLUMN "aliyunDescribeInstances"."%s" IS '%s'`, v[0], v[1])
...
func writeSql(resStr string) {
	...
	for _, v := range p.Instance {
		sqlData := fmt.Sprintf(`INSERT INTO "aliyunDescribeInstances"("ResourceGroupId","Memory","Cpu","OSName","ImageId","EipAddress","HostName","Status","VpcAttributes","InstanceId","InstanceName","InternetMaxBandwidthOut","OSType","Description","PublicIpAddress","RegionId") VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')`,
			v.ResourceGroupId,
