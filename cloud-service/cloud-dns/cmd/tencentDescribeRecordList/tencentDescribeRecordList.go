...
func Tosql() {
	for _, v := range tencentDescribeDomainList.Get() {
		if v[1] == "ENABLE" {
			var data string
			data = getRecordList(v[0], "A")
			if data == "" {
				continue
			}
			writeSql([]byte(data), v[0])
			data = getRecordList(v[0], "CNAME")
			if data == "" {
				continue
			}
			writeSql([]byte(data), v[0])
...
func CheckSqlTable() {
	...
	for rows.Next() {
		...
		if count == "1" {
			...
			// 检查RecordId列类型并修改为BIGINT
			alterData := `ALTER TABLE "tencentDomainList" ALTER COLUMN "RecordId" TYPE BIGINT`
			rows3, err := db.Query(alterData)
			if err != nil {
				log.Printf("修改RecordId列类型失败（可能已经是BIGINT）：%v", err)
			} else {
				log.Println("RecordId列类型已修改为BIGINT")
				rows3.Close()
			}
		} else {
	...
	for _, i := range p.Response.RecordList {
		if i.Name == "@" {
			sqlData := fmt.Sprintf(`INSERT INTO "tencentDomainList"("Line","LineId","MX","MonitorStatus","Name","Record","Domain","RecordId","Remark","Status","TTL","Type","UpdatedOn","Value") VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')`,
				...
				domain,
				domain,
				strconv.FormatInt(i.RecordId, 10),
				i.Remark,
				i.Status,
				strconv.Itoa(i.TTL),
				i.Type,
